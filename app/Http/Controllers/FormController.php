...

{
     ...
     public function index()
    {
      ...
      $fechas = Fechas::all();
      return view('form',["fechas"=>$fechas]);
    }
    ...
    public function listar_datos()
    {
      ...
      $tabla='<table id="lista">
        ...
        <thead>
           <tr>
             ...
             <th style="text-transform: capitalize;">Fecha de la caminata</th>           
          </tr>
        </thead>
        <tbody id="tabla">';
      ...
      foreach ($acceso as $key => $value)
      {
       ...
       $tabla.='<td>'.$value->fechas['fecha'].'</td></tr>';
      }
    ...
    public function insertar(Request $request)
    {
      ...
      $formulario = new Form([]);
        ...
        //envio de correo
       $registros = $this->inscritos($request->input('fecha'));
        var_dump($registros->count());

        if($registros->count() <40){
        ...
        return view('error', ['error' => 'Lo sentimos el limite de inscritos  en esta fecha fue superado!']);
      }
...
// conteo de la tabla
    private function inscritos($fecha){
      $registros = Form::where('fecha', $fecha)->get();
      return $registros;
    }
